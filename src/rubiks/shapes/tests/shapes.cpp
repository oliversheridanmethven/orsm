...
TYPED_TEST(ShapeTest, shuffle)
{
    ...
    {
        TypeParam shape;
        auto [shuffled, path] = shuffle(shape, turns, std::nullopt);
...
TYPED_TEST(ShapeTest, shuffle_seeding)
{
    ...
    Path path_1, path_2;
    auto turns = 10;
    const std::vector<Seed> seeds = {std::nullopt, 0, 1, 2};
    for (const auto &seed: seeds)
    {
        while (true)
        {
            std::tie(shuffle_1, path_1) = shuffle(shape_1, turns, seed);
            std::tie(shuffle_2, path_2) = shuffle(shape_2, turns, seed);
...
TYPED_TEST(ShapeTestPrivate, invariant_positions)
{
    ...
    {
        auto [shuffled, path] = shuffle(shape, turns, seed);

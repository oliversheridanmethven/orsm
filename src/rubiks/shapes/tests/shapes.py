...
from rubiks.colours.colour_palette import ColourPalette
...
class BasicProperties(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cubic_shapes = variable_names_and_objects(shapes.Volume, shapes.Cube)
...
class Moves(unittest.TestCase):
    ...
    def test_single_moves_cubic(self):
        for name, Shape in self.cubic_shapes:
            shape = Shape()
            for direction in [True, False]:
                other_direction = not direction
                for move in shape.moves():
                    shape_moved_once = shape.move(move, reverse=direction)
                    shape_moved_reverted = shape_moved_once.move(move, reverse=other_direction)
                    self.assertEqual(shape_moved_reverted, shape, f"{shape_moved_reverted = !s}, {shape = !s} should be equal when using {move = }")

    def test_double_moves_cubic(self):
        for name, Shape in self.cubic_shapes:
            shape = Shape()
            for move_1, move_2 in progressbar(list(itertools.product(*[shape.moves() for i in range(2)]))):
                for direction in [False, True]:
                    ...
                    self.assertEqual(shape_moved_twice_reverted, shape_moved_once, f"{shape_moved_twice_reverted = !s}, {shape_moved_once = !s} should be equal when using {move_1 = } followed by {move_2 = }")
                    shape_reverted_in_order = shape_moved_twice.move(move_2, move_1, reverse=other_direction)
                    self.assertEqual(shape_reverted_in_order, shape, f"Expected shapes {shape_reverted_in_order = !s} and {shape = !s} to be equal when performing {move_1 = } and {move_2 = } in {direction = }")
    ...
    def test_moves_give_new_objects(self):
        for name, Shape in self.cubic_shapes:
            shape = Shape()
            for moves in progressbar(list(all_combinations(shape.moves(), max_length=2))):
